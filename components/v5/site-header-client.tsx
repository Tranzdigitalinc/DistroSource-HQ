...
export function V5SiteHeaderClient({ departments }: { departments: Departments }) {
  ...
  const visible = departments.filter((d) => d.productCount > 0).slice(0, 8)
    ...
    <>
      <header className="sticky top-0 z-50 border-b border-black/[0.06] bg-[color:var(--background)]/94 backdrop-blur-xl dark:border-white/10">
        <div className="mx-auto flex h-[76px] max-w-[1600px] items-center gap-4 px-4 sm:px-6 lg:px-8">
          ...
          <Link href="/" className="shrink-0" aria-label="DistroSource home">
          ...
          <nav className="ml-4 hidden items-center gap-1 lg:flex" aria-label="Primary navigation">
            <button
              ...
              className="inline-flex h-10 items-center px-3 text-sm font-semibold tracking-[-0.01em] text-foreground/80 transition-colors hover:text-foreground"
            >
              Categories
            </button>
        ...
        <AnimatePresence>
          {megaOpen && (
            <motion.div
              ...
              className="absolute inset-x-0 top-full border-b border-black/[0.06] bg-background shadow-[0_30px_80px_-35px_rgba(0,0,0,.28)] dark:border-white/10"
            >
                  ...
                  {visible.map((department) => (
                    <div key={department.slug}>
                      ...
                      <div className="mt-2 space-y-1.5">
                        {department.subcategories.filter((s) => s.productCount > 0).slice(0, 4).map((subcategory) => (
                          <Link key={subcategory.slug} href={`/categories/${subcategory.slug}`} onClick={() => setMegaOpen(false)} className="block text-xs leading-5 text-muted-foreground transition-colors hover:text-foreground">{subcategory.name}</Link>
                        ))}
                      </div>
                    </div>
                  ))}
          ...
          <motion.div className="fixed inset-0 z-[100] bg-background" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <div className="flex h-full flex-col">
              <div className="flex h-[76px] items-center justify-between border-b border-border px-4">
                <BrandLogo href="/" heightClassName="h-8" />
                <button type="button" onClick={() => setMobileOpen(false)} className="flex size-11 items-center justify-center" aria-label="Close navigation"><Close size={20} /></button>
              </div>
              <div className="border-b border-border p-4"><HeaderSearch size="lg" /></div>
              <div className="min-h-0 flex-1 overflow-y-auto px-4 py-6">
                <div className="space-y-1">
                  <Link href="/products" onClick={() => setMobileOpen(false)} className="flex min-h-14 items-center justify-between border-b border-border text-xl font-black">All products <ArrowRight size={16} /></Link>
                  <Link href="/gaming" onClick={() => setMobileOpen(false)} className="flex min-h-14 items-center justify-between border-b border-border text-xl font-black">Gaming <ArrowRight size={16} /></Link>
                  <Link href="/deals" onClick={() => setMobileOpen(false)} className="flex min-h-14 items-center justify-between border-b border-border text-xl font-black">Deals <ArrowRight size={16} /></Link>
                </div>
                <p className="mb-3 mt-8 font-mono text-[10px] font-bold uppercase tracking-[0.14em] text-primary">Departments</p>
                <div className="grid grid-cols-2 gap-2">
                  {visible.map((department) => (
                    <Link key={department.slug} href={`/categories/${department.slug}`} onClick={() => setMobileOpen(false)} className="min-h-24 border border-border p-4">
                      <span className="font-display text-base font-black">{department.name}</span>
                      <span className="mt-3 block font-mono text-[10px] text-muted-foreground">{department.productCount} products</span>
                    </Link>
                  ))}
                </div>
              </div>
