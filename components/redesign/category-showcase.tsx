...
export function RedesignCategoryShowcase({
  categories,
}: {
  ...
  return (
    <section className="mx-auto max-w-[1500px] px-6 py-16 sm:px-8 lg:px-10 lg:py-24">
      <div className="mb-10 flex flex-col gap-5 border-b border-border pb-7 sm:flex-row sm:items-end sm:justify-between">
        <div className="max-w-2xl">
          ...
          <h2 className="mt-3 font-display text-4xl font-black tracking-[-0.04em] text-foreground sm:text-5xl">
            Start with what you need.
          </h2>
          <p className="mt-3 text-sm leading-6 text-muted-foreground sm:text-base">
            Broad enough to explore, structured enough to find the right asset quickly.
          </p>
        </div>

        <Link
          href="/categories"
          className="inline-flex min-h-11 items-center gap-2 self-start border border-border px-4 py-2.5 text-sm font-semibold text-foreground transition-colors hover:border-primary/50 hover:text-primary sm:self-auto"
        >
        ...
        {visible.map((category, index) => {
          ...
          return (
            <Link
              ...
              className="group relative min-h-52 overflow-hidden bg-background p-6 transition-colors duration-200 hover:bg-secondary/45"
            >
              <div className="flex items-start justify-between gap-4">
                <span className="flex size-11 items-center justify-center border border-border bg-card text-primary transition-transform duration-200 group-hover:-translate-y-1">
                  <Icon className="size-5" aria-hidden="true" />
                </span>
                <span className="font-mono text-[10px] font-semibold text-muted-foreground/65">
