...
export function businessApp({ tab = "staff" } = {}) {
  let content
  if (tab === "payroll") {
    content = `<div class="two"><div class="card"><table><thead><tr><th>Employee</th><th>Role</th><th>Hours this cycle</th><th>Accrued</th><th>Last paid</th></tr></thead><tbody>${[["Dana Whitfield", "Manager", "6.5 h", "$1,430", "Today 12:00"], ["Priya Raman", "Barista", "5.0 h", "$800", "Today 12:00"], ["Tom Ferreira", "Barista", "2.5 h", "$400", "Yesterday 18:00"]].map((r) => `<tr><td><b>${r[0]}</b></td><td><span class="tag">${r[1]}</span></td><td class="mono">${r[2]}</td><td class="mono" style="color:#7fe0a3">${r[3]}</td><td style="color:#8b93a1">${r[4]}</td></tr>`).join("")}</tbody></table></div>
      <div class="panel"><div class="lbl">Payroll settings</div><div class="kv"><span>Interval</span><b>every 60 min</b></div><div class="kv"><span>Pay only on duty</span><b>yes</b></div><div class="kv"><span>Next run</span><b>in 23 min</b></div><div class="kv"><span>Source account</span><b>Business · $12,480</b></div><div class="kv"><span>Projected this cycle</span><b style="color:#f39c93">$2,630</b></div><div style="margin-top:14px"><span class="btn p">Run payroll now</span></div></div></div>`
  } else if (tab === "roles") {
    const perms = ["Sell", "View stock", "Order stock", "Set wages", "Hire / fire", "View books", "Withdraw"]
    const roles = [["Owner", [1, 1, 1, 1, 1, 1, 1]], ["Manager", [1, 1, 1, 1, 1, 1, 0]], ["Supervisor", [1, 1, 1, 0, 0, 1, 0]], ["Barista", [1, 1, 0, 0, 0, 0, 0]], ["Trainee", [1, 0, 0, 0, 0, 0, 0]]]
    content = `<div class="card"><table><thead><tr><th>Role</th>${perms.map((p) => `<th>${p}</th>`).join("")}<th>Wage</th></tr></thead><tbody>${roles.map((r, i) => `<tr><td><b>${r[0]}</b></td>${r[1].map((v) => `<td>${v ? '<span class="tag ok">yes</span>' : '<span class="tag" style="opacity:.45">—</span>'}</td>`).join("")}<td class="mono">${["—", "$220 / h", "$190 / h", "$160 / h", "$120 / h"][i]}</td></tr>`).join("")}</tbody></table></div>`
  } else if (tab === "stock") {
  ...
  const body = app({ brand: "Harbour Coffee", nav: ["Staff", "Payroll", "Stock", "Books", "Roles"], active: { staff: 0, payroll: 1, stock: 2, books: 3, roles: 4 }[tab] ?? 0, title: { staff: "Staff", payroll: "Payroll", stock: "Stock", books: "Books", roles: "Roles" }[tab] ?? "Staff", stats: [["Staff", "3"], ["On duty", "2"], ["Balance", "$12,480"]], content })
  return doc({ body, style: APP_STYLE, bg: "#3b4a5c" })
}
