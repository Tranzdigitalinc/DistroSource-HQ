...
import { getBrands, getCategories, getProducts } from "@/lib/queries/catalog"

const baseUrl = "https://redeemcove.com"

export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  const [products, brands, categories] = await Promise.all([getProducts({ limit: 5000 }), getBrands(), getCategories()])
  return [
